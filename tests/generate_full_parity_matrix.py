...
FEATURES = {
    ...
    'pick_song': ('网易云点歌/#听N', '搜索 API + 会话缓存 + pick-song 图片复刻 + 音频链接', 'test_core_services.py, test_style_quantitative.py'),
    ...
    'cloud_upload': ('上传云盘', '入口保留；需 Cookie、文件上传、真实账号授权', 'parity_matrix + adapter_capability_probe'),
    ...
    'douyin': ('抖音解析', 'yt-dlp metadata/direct/download + OpenGraph fallback；私密/评论/BGM 需 Cookie', 'test_media_resolvers.py + external_workflow_validation.md'),
    'tiktok': ('TikTok 解析', 'yt-dlp metadata/direct/download', 'test_media_resolvers.py'),
    'bili_scan': ('B站扫码登录', '调用 Bilibili 官方二维码生成 API，发送二维码图片，保存 qrcode_key', 'test_bilibili_auth.py + stub e2e'),
    'bili_state': ('B站登录状态', '轮询二维码扫码状态；成功后保存 SESSDATA/Cookie 到插件数据目录', 'test_bilibili_auth.py + config schema'),
    'bili': ('B站解析', 'Bilibili API 基础信息 + yt-dlp 媒体增强 + 官方 playurl 本地视频下载 fallback', 'test_core_services.py, test_media_resolvers.py, test_bilibili_video.py'),
...
def main():
    ...
    for i,(name,pattern,handler,perm,source) in enumerate(rows,1):
        ...
        data.append({
            ...
            'remaining_runtime_requirement': '真实账号/适配器/平台 Cookie' if status_for(name).startswith('环境') else '无沙箱内阻塞'
        })
    OUT_JSON.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding='utf-8')
    lines=['# 原 R 插件 → AstrBot 版完整逐规则功能矩阵', '', '本矩阵由 `tests/generate_full_parity_matrix.py` 从 AstrBot 版 `main.py::_build_rules()` 自动生成并核对，共 46 条。聊天内插件更新入口已按产品要求移除，版本入口保留。', '', '| # | 原模块 | 原规则名 | 原正则 | 权限 | AstrBot handler | 功能 | 状态 | AstrBot 实现 | 验证证据 | 剩余运行时要求 |', '|---:|---|---|---|---|---|---|---|---|---|---|']
    for r in data:
        def esc(x): return str(x).replace('|','\\|').replace('\n','<br>')
        lines.append(f"| {r['index']} | `{esc(r['source_module'])}` | `{esc(r['rule_name'])}` | `{esc(r['original_regex'])}` | {esc(r['permission'])} | `{esc(r['astrbot_handler'])}` | {esc(r['feature'])} | {esc(r['status'])} | {esc(r['astrbot_implementation'])} | {esc(r['verification_evidence'])} | {esc(r['remaining_runtime_requirement'])} |")
    lines += ['', '## 结论', '', '- 46 条运行入口均在 AstrBot 版存在对应 handler；聊天内插件更新入口已移除。', '- 平台无关功能已实现并通过单元/stub/样式/媒体解析测试。', '- 必须依赖真实账号、扫码二维码、Cookie、群文件/群语音或具体适配器的能力标记为“环境依赖/入口完整”，不伪造成功；插件保留入口、配置与安全提示，并通过能力探针报告说明可验证条件。']
