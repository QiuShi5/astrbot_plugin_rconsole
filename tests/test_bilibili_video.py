...
async def main():
    ...
    resolver.media.extract = lambda url, platform="": asyncio.sleep(
        0, result=ROutput(text="yt-dlp 解析链已在配置中关闭。")
    )
    resolver.bili_video.extract_video = lambda **kwargs: asyncio.sleep(
        0, result=ROutput(text="native ok", videos=[str(temp / "native.mp4")])
    )
    ...
    async def fake_json(url, timeout=15, headers=None):
        return {
            ...
            "data": {
                ...
                "cid": 123,
                "stat": {"view": 10000, "like": 2, "coin": 3, "favorite": 4, "share": 5, "danmaku": 6, "reply": 7},
            },
        }
    ...
    async def fake_short_json(url, timeout=15, headers=None):
        ...
        return {
            ...
            "data": {
                ...
                "pic": "",
                "cid": 456,
                "stat": {"view": 3, "like": 4, "coin": 5, "favorite": 6, "share": 7, "danmaku": 8, "reply": 9},
            },
        }
