...
async def main():
    ...
    # Public YouTube test video metadata path; tolerate network/extractor failure but
    # require the service to produce a structured ROutput rather than crash.
    out = await resolver.resolve('youtube', 'https://www.youtube.com/watch?v=BaW_jenozKc')
    ...
    direct = await direct_ytdlp._output_from_info({
        ...
        'filesize': 1024 * 1024,
    }, platform='通用直链视频', original_url='https://example.com/video')
    ...
    assert resolved_douyin.videos == ['https://aweme.snssdk.com/aweme/v1/play/?video_id=video-uri-123&ratio=1080p&line=0']

    short_douyin = ResolverService(temp_dir=temp, ytdlp_mode='metadata', douyin_cookie='sessionid=x')
    short_douyin._expand_douyin_url = lambda url: asyncio.sleep(0, result=('https://www.douyin.com/video/7651544681980906803', 'short-ttwid'))
    ...
    missing = await xhs_without_cookie.resolve_xhs('https://www.xiaohongshu.com/explore/abc123?xsec_token=token123&xsec_source=pc_feed')
    ...
    note = {
        ...
        'desc': '小红书测试简介',
        'type': 'normal',
        'imageList': [{'urlDefault': '//sns-img.example.com/1.jpg'}, {'urlDefault': 'https://sns-img.example.com/2.jpg'}],
    }
